...
sample_name = "PBMCs_LRAA-init"

sparse_matrix_data_dir = "../../LRAA_PBMCs^isoform-sparseM/"

umap_cluster_file = "LRAA_sc_PBMCs.genes-cell_cluster_assignments.wUMAP.wCAS.tsv"

cluster_pseudobulk_matrix_filename = "LRAA_PBMCs^isoform-sparseM.clusters_pseudobulk.matrix"

gtf_filename = "../../PBMCs_pbio.LRAA.gtf.updated.gtf.segmented.gtf"

diff_iso_usage_stats_filename = "LRAA_PBMCs^isoform-sparseM.top_only_w_recip_delta_pi.minCF0.05.diff_iso.tsv.signif_only"
...
plot_isoform_umap = function(gene_of_interest, restrict_to_transcript_ids = NULL) {
  ...
  max_color_value = quantile(isoform_umap$log_read_count, 0.95, na.rm = TRUE)
  
  base_umap + geom_point(data=isoform_umap, aes(color=log_read_count)) +
...
# how many genes identified with DTU?

diff_iso_usage_stats %>% select(gene_id) %>% unique() %>% nrow()
...
p = make_diff_iso_usage_compound_plot(top_DTU_results_w_same_splicehash$gene_id[1], MIN_DELTA_PI, ignore_unspliced = FALSE)
