...
```{r}
features_per_cell = colSums(data >0)
features_per_cell = sort(features_per_cell, decreasing = T)
plot(features_per_cell, log='xy')
```
...
seurat_obj[["percent.mt"]] <- PercentageFeatureSet(seurat_obj, pattern = "^(MT-|mt-|g:(chrM|MT|M):)")
...
library(future)
# Set the plan for parallelization
plan(multisession)
options(future.globals.maxSize = 10000 * 1024^2) # Set to 10B (10000 MB)
seurat_obj = ScaleData(seurat_obj, vars.to.regress = c("nFeature_RNA", "nCount_RNA", "percent.mt"))
...
perform_pairwise_de <- function(seurat_obj, group_by = "seurat_clusters", 
...
# Run the analysis
pairwise_de_results <- perform_pairwise_de(seurat_obj)
