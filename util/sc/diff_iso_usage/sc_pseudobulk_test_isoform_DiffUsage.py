...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "--sc_cluster_fraction_matrix file specifications (but not --splice_hashcode_id_mappings)."
        ),
    )
    ...
    if args.resources_dir:
        ...
        # splice_hashcode_id_mappings must still be provided explicitly
        splice_hashcode_id_mappings_file = args.splice_hashcode_id_mappings
        if not splice_hashcode_id_mappings_file:
            logger.error("--splice_hashcode_id_mappings must be provided (not auto-detected from resources_dir)")
            sys.exit(1)
    else:
        ...
        if not sc_cluster_counts_matrix:
            ...
            sys.exit(1)
        if not splice_hashcode_id_mappings_file:
            logger.error("--splice_hashcode_id_mappings is required")
            sys.exit(1)
