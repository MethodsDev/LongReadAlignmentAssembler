...
def path_A_contains_path_B(simple_path_A, simple_path_B):
    ...
    while idx_B < len(simple_path_B):
        ...
        idx_A += 1
        if (
            idx_B < len(simple_path_B)
            and idx_A < len(simple_path_A)
            and simple_path_A[idx_A] != simple_path_B[idx_B]
        ):
            return False
    ...
    for idx_A in range(0, len(simple_path_A)):
        ...
        if A_node_id in simple_path_B:
            ...
            while idx_A < len(simple_path_A) and idx_B < len(simple_path_B):
                if (
                    simple_path_A[idx_A] == SPACER
                    or simple_path_A[idx_A] != simple_path_B[idx_B]
                ):
                    return False
                idx_A += 1
                idx_B += 1
...
def merge_adjacent_segments(segment_list):
    ...
    while len(segment_list) > 0:
        ...
        if next_seg[0] == prev_seg[1] + 1:
            ...
            raise RuntimeError(
                "Error, not sure hwo to merge adjacent segments: {} and {}".format(
                    prev_seg, next_seg
                )
            )
...
def _convert_path_to_nodes_with_coords_list(
    sg: Splice_graph, simple_path: list, ret_node_objs=False
) -> list:
...
def merge_simple_paths_containing_spacers(
    sg: Splice_graph, simple_path_A: list, simple_path_B: list
) -> list:
    """
...
def simple_path_A_within_bounds_of_simple_path_B(
    sg: Splice_graph, simple_path_A: list, simple_path_B: list
) -> bool:
...
def simple_path_A_contains_and_compatible_with_simple_path_B_spacefree_region_path_A(
    sg: Splice_graph, simple_path_A: list, simple_path_B: list
) -> bool:

    if simple_path_A == simple_path_B:
        return True
    elif path_A_contains_path_B(
        remove_spacers_from_path(simple_path_A), remove_spacers_from_path(simple_path_B)
    ):
        return True
    else:
        return simple_path_A_within_bounds_of_simple_path_B(
            sg, simple_path_B, simple_path_A
        ) and simple_paths_overlap_and_compatible_spacefree_region_path_A(
            sg, simple_path_A, simple_path_B
        )


def simple_path_A_contains_and_compatible_with_simple_path_B_spacer_aware_both_paths(
    sg: Splice_graph, simple_path_A: list, simple_path_B: list
) -> bool:
    ...
    if simple_path_A == simple_path_B:
        ...
        return simple_path_A_within_bounds_of_simple_path_B(
            sg, simple_path_B, simple_path_A
        ) and simple_paths_overlap_and_compatible_spacer_aware_both_paths(
            sg, simple_path_A, simple_path_B
        )


def simple_paths_overlap_and_compatible_spacefree_region_path_A(
    sg: Splice_graph, simple_path_A: list, simple_path_B: list
) -> bool:
    ...
    while len(my_path) > 0 and len(other_path) > 0:
        ...
        if coordpairs_overlap(my_node_obj.get_coords(), other_node_obj.get_coords()):
            ...
            if (
                other_path_spacer_mode
                and my_node_obj.get_coords()[0] < other_node_obj.get_coords()[0]
            ):
                my_path.pop(0)
            elif (
                my_path_spacer_mode
...
def simple_paths_overlap_and_compatible_spacer_aware_both_paths(
    sg: Splice_graph, simple_path_A: list, simple_path_B: list
) -> bool:
    ...
    for i in range(1, len(simple_path) - 1):
        if simple_path[i] == SPACER:
            ...
            if next_node_obj in sg.get_successors(prev_node_obj):
                ...
                logger.debug(
                    "-deleting spacer between connected nodes {} and {} in path {}".format(
                        prev_node_id, next_node_id, simple_path
                    )
                )
...
def refine_TSS_simple_path(splice_graph, simple_path):
    ...
    nodes_with_coords_list = _convert_path_to_nodes_with_coords_list(
        splice_graph, simple_path
    )
    ...
    logger.debug(
        "Found TSS in path {} at indices {}".format(nodes_with_coords_list, TSS_indices)
    )
    ...
    if contig_strand == "+" and TSS_indices[0] != 0:
        ...
        if lend_coord == rend_coord and lend_coord == TSS_coord and TSS_index == 1:
            # swap order
            (nodes_with_coords_list[0], nodes_with_coords_list[1]) = (
                nodes_with_coords_list[1],
                nodes_with_coords_list[0],
            )
        ...
        if (
            ...
            and rend_coord == TSS_coord
            and TSS_index == len(nodes_with_coords_list) - 2
        ):
            # swap
            (nodes_with_coords_list[-2], nodes_with_coords_list[-1]) = (
                nodes_with_coords_list[-1],
                nodes_with_coords_list[-2],
            )
...
def refine_PolyA_simple_path(splice_graph, simple_path):
    ...
    logger.debug(
        "Found POLYA in path {} at indices {}".format(
            nodes_with_coords_list, polyA_indices
        )
    )
    ...
    if contig_strand == "+" and polyA_indices[-1] != len(nodes_with_coords_list) - 1:
        ...
        if (
            lend_coord == rend_coord
            and rend_coord == polyA_coord
            and polyA_index == len(nodes_with_coords_list) - 2
        ):
            # swap
            (nodes_with_coords_list[-2], nodes_with_coords_list[-1]) = (
                nodes_with_coords_list[-1],
...
def add_spacers_between_disconnected_nodes(splice_graph, simple_path):
    ...
    if spacer_added:
        logger.debug(
            "-spacer added between unconnected nodes. Prev path:\n{}\nnew path\n{}".format(
                simple_path, new_path
            )
        )
...
def test_overlapping_n_compatible_spacers_aware_both_paths():
    ...
    assert (
        simple_paths_overlap_and_compatible_spacer_aware_both_paths(sg, p1, p2) == True
    )
    assert (
        simple_paths_overlap_and_compatible_spacer_aware_both_paths(sg, p2, p1) == True
    )

    # test incompatible paths - no spacers
    p3 = ["E:1", "E:2", "E:4"]
    assert (
        simple_paths_overlap_and_compatible_spacer_aware_both_paths(sg, p1, p3) == False
    )
    assert (
        simple_paths_overlap_and_compatible_spacer_aware_both_paths(sg, p3, p1) == False
    )
    ...
    assert (
        simple_paths_overlap_and_compatible_spacer_aware_both_paths(sg, sp1, sp2)
        == True
    )

    sp3 = ["E:1", SPACER, "E:3"]
    assert (
        simple_paths_overlap_and_compatible_spacer_aware_both_paths(sg, sp1, sp3)
        == True
    )
    ...
    p4 = ["E:2", "E:3", "E:5"]
    assert (
        simple_paths_overlap_and_compatible_spacer_aware_both_paths(sg, sp2, p4)
        == False
    )
    ...
    assert (
        simple_paths_overlap_and_compatible_spacer_aware_both_paths(sg, sp4, sp5)
        == True
    )
    ...
    p5 = ["E:1", "E:2", "E:5"]
    assert (
        simple_paths_overlap_and_compatible_spacer_aware_both_paths(sg, p5, sp4)
        == False
    )
...
def test_simple_paths_overlap_and_compatible_spacefree_region_path_A():
    ...
    # test compatible paths - no spacers
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, p1, p2) == True
    )
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, p2, p1) == True
    )

    # test incompatible paths - no spacers
    p3 = ["E:1", "E:2", "E:4"]
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, p1, p3) == False
    )
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, p3, p1) == False
    )
    ...
    sp2 = ["E:1", SPACER, "E:3", "E:4"]
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, sp1, sp2)
        == False
    )  ##

    sp3 = ["E:1", SPACER, "E:3"]
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, sp1, sp3)
        == False
    )  ##
    ...
    p4 = ["E:2", "E:3", "E:5"]
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, sp2, p4)
        == False
    )
    ...
    sp5 = ["E:1", "E:2", "E:3", "E:4", SPACER, "E:5"]
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, sp4, sp5)
        == False
    )  ##
    ...
    p5 = ["E:1", "E:2", "E:5"]
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, p5, sp4)
        == False
    )
    ...
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, sp5, p1) == True
    )

    sp6 = ["E:2", "E:3", "E:4", SPACER, "E:5"]
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, sp6, p1) == True
    )

    p6 = ["E:3", "E:4", "E:5"]
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, p6, sp6) == True
    )
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, sp6, p6)
        == False
    )

    p7 = ["E:2"]
    assert (
        simple_paths_overlap_and_compatible_spacefree_region_path_A(sg, sp3, p7)
        == False
    )
