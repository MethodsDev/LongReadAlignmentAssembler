    ...
    def add_commands(self, cmds_list):
        ...
        for cmd in cmds_list:
            ...
            if not (isinstance(cmd, Command) or isinstance(cmd, ParallelCommandList)):
                errmsg = "Pipeliner::add_commmands - Error, cmd {} is not a Command or ParallelCommandList object".format(
                    cmd
                )
    ...
    def run(self, checkpoint_dir):
        ...
        if os.path.exists(checkpoint_file):
            ...
            if ret:
                # failure occurred.
                errmsg = str(
                    "Error, command: [ {} ] failed, stack trace: [ {} ] ".format(
                        cmdstr, self.get_stacktrace()
                    )
                )
                logger.error(errmsg)
                ...
                logger.info(
                    "Execution Time = {:.2f} minutes. CMD: {}".format(
                        runtime_minutes, cmdstr
                    )
                )
...
class ParallelCommandList(object):
    ...
    def run(self, checkpoint_dir):
        ...
        full_path_parallel_job_checkpoint_file = os.path.sep.join(
            [checkpoint_dir, parallel_job_checkpoint_file]
        )
        if os.path.exists(full_path_parallel_job_checkpoint_file):
            logger.info(
                "Parallel command series already completed, so skipping. Checkpoint found as: {}".format(
                    full_path_parallel_job_checkpoint_file
                )
            )
            return
        ...
        while self._num_running > 0:
            time.sleep(
                1
            )  # avoid unnecessarily running at 100% CPU. Sleeping 1 second is a long enough time in the quantum realm.
            iter_counter += 1
            if iter_counter % 60 == 0:
                sys.stderr.write(
                    "\r waiting for {} jobs to complete.    ".format(self._num_running)
                )
