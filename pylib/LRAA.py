...
class LRAA:
    ...
    def build_multipath_graph(self, contig_acc, contig_strand, contig_seq, bam_file, allow_spacers=False):
        ...
        multipath_graph = MultiPathGraph(mp_counter, self._splice_graph, contig_acc, contig_strand, LRAA.min_mpgn_read_count, allow_spacers)
    ...
    def reconstruct_isoforms(self, single_best_only=False):
        ...
        mpg_components = mpg.remove_small_components(mpg_components, LRAA.min_transcript_length)
        logger.info("{} components surviving the min length {} criterion.".format(len(mpg_components), LRAA.min_transcript_length))
        ...
        for mpg_component in mpg_components:
            ...
            logger.info("LRAA - assembly of component {} region: {}:{}-{}".format(component_counter, self._contig_acc, coord_span[0], coord_span[1]))
    ...
    def _map_segment_to_graph_SINGLE(self, segment):
        ...
        overlapping_segments = self._splice_graph.get_overlapping_exon_segments(segment[0], segment[1], min_frac_feature_overlap=LRAA_Globals.config['min_feature_frac_overlap'])
    ...
    def _map_segment_to_graph_INITIAL(self, segment):
        ...
        for i, exon_segment in enumerate(overlapping_segments):
            ...
            if exon_segment._rend == segment[1]:
                ...
                if frac_feature_overlap >= LRAA_Globals.config['min_feature_frac_overlap']:
                    path.append(exon_segment.get_id())
