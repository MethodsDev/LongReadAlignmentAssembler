...
Isoform abundances are estimated using an expectation-maximization (EM) algorithm that resolves ambiguous read assignments. Read-isoform compatibility is determined by splicing agreement: the read's labeled read path must match the isoform's exon-intron structure. Exact matches are preferred; otherwise, reads overlapping at least 75% of their length with an isoform's structure are considered compatible. Reads are additionally weighted by their 3' end proximity to isoform 3' termini to account for alternative polyadenylation. For a read compatible with multiple isoforms, the weight assigned to each isoform $i$ is proportional to $1 - d_i / D$, where $d_i$ is the genomic distance between the read's 3' end and isoform $i$'s 3' terminus, and $D$ is the sum of distances across all compatible isoforms. This linear weighting scheme prioritizes isoforms whose 3' ends align more closely with the read's observed termination point.
